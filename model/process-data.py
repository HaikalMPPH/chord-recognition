...
def featurize_file(audio_filename, label_filename, segment_duration_sec):
    try:
        ...
        if False: # used when testing
            ...
            #chroma_harm = librosa.feature.chroma_stft(y=y_harm, sr=sr, hop_length=hop_length_samples)
        ...
        chroma_fps = sr / hop_length_samples
        ...
        for idx, seconds_start, seconds_end, label in df_labels.itertuples():
            ...
            overall_chroma_start_idx = int(np.round(seconds_start * chroma_fps))
            overall_chroma_end_idx = int(np.round(seconds_end * chroma_fps))
            ...
            for frame_idx in range(overall_chroma_start_idx, overall_chroma_end_idx):
                ...
                features_list.append(chroma_100ms_segment)
...
def main():
    ...
    print(f'{len(df_master[CHORD_LABEL_COLUMN].unique())} Unique chord labels in dataset: {df_master[CHORD_LABEL_COLUMN].unique()}')
