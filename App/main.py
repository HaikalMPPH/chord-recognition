...
TONNETZ_MIN_MAX = "../Model/tonnetz_min_max.npz"
...
if __name__ == "__main__":
        ...
        if uploaded_file is not None:
                ...
                with st.spinner("Processing audio..."):
                        ...
                        # Tonnetz (with normalization)
                        #tonnetz = librosa.feature.tonnetz(y=y_harm, sr=sr, hop_length=hop_length)
                        #tonnetz_stat = np.load(TONNETZ_MIN_MAX)
                        #tonnetz_min = tonnetz_stat["min"]
                        #tonnetz_max = tonnetz_stat["max"]
                        #tonnetz = (tonnetz - tonnetz_min[:, np.newaxis]) / (tonnetz_max[:, np.newaxis] - tonnetz_min[:, np.newaxis] + 1e-6)


                        #features = chroma.T
                        features = []
                        for idx in range(0, chroma.shape[1]):
                            features.append(np.hstack((chroma[:, idx], chroma_cens[:, idx])))
                        ...
                        prediction_df = pd.DataFrame({
                                "start": start_time,
                                "end": end_time,
                                #"chord": ENCODER.inverse_transform(
                                #        np.argmax(MODEL.predict(features), axis=1)
                                #),
                                #"start": start_time_seq,
                                #"end": end_time_seq,
